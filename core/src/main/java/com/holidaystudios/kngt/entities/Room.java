...
public class Room {
    ...
    private void connectDoors(final Map<DoorPosition, Integer> _doors) {
        ...
        final Integer corridorBreadth = Math.max(5, (int) Math.round(Math.random()*7));
        ...
        switch (availableDoors.length) {
            case 1:
                final Rectangle dim = new Rectangle(
                    0,
                    0,
                    Math.round(Cave.MIN_WALL_LENGTH + Math.random() * (Cave.MAX_WALL_LENGTH - Cave.MIN_WALL_LENGTH)),
                    Math.round(Cave.MIN_WALL_LENGTH + Math.random() * (Cave.MAX_WALL_LENGTH - Cave.MIN_WALL_LENGTH))
                );
